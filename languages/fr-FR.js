  ...
  Please respond with the number of the user you want.`.stripIndents(),
    ...
    **Deleting**: ❌ or 🇽 - only on starred messages they own
    ...
    While you wait, why not ${ waitSuggestions.join(' or ') }!`.stripIndents(),
      ...
      ${ showThisBit ? `You can add/subtract to the amount of stars needed for an existing reward role by creating it again, but putting a +/- before the stars argument. (+50, -50 etc)
...
${ help ? `
      ...
      Say \`cancel\` or \`stop\` to cancel the setup, \`skip\` or \`next\` to skip to the next setting, and \`end\` to skip to the end.`.stripIndents(),
            ...
            RESULT: (b, p, c, v) => `${ b ? `Successfully updated the settings for this ${ c ? 'channel' : 'server' }` : `The settings for this ${ c ? 'channel' : 'server' } have not been updated` }. If you want to change more settings, you can view them all with ${ p }settings${ v ? '' : `, and if you want to change the emoji, you must vote for the bot first and then either run this command again or run \`${ p }changesetting emoji <[emoji]>\`` }.`
        },
        MESSAGEINFO: {
