...
module.exports = {
    ...
    REFERENCED_MESSAGE: (isReply, deleted) => isReply ? `Replying to this message${ deleted ? ' (deleted)' : '' }` : 'Referenced Message',
    ...
    **Saving**: 📥
    ...
    While you wait, why not ${ waitSuggestions.join(' or ') }!`.stripIndents(),
        ...
        HELP: {
            ...
            USAGE: 'help (command)',
            EMBED_DESCRIPTION: (command, owner, categories, prefix) => `Commands: ${ owner ? command.client.commands.size : Object.values(categories).reduce((p, c) => c.split('\n').length + p, 0) }
